...
import (
	...
	"github.com/tucats/apitest/data"
	"github.com/tucats/apitest/errors"
)
...
func Define(key string, object any) error {
	if strings.HasPrefix(key, privateTypePrefix) {
		panic("Invalid validation definition using private prefix: " + key)
	}
...
func Decode(b []byte) error {
	...
	for key, value := range m {
		if strings.HasPrefix(key, privateTypePrefix) {
			panic("Invalid validation definition using private type prefix: " + key)
		}
